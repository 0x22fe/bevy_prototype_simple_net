...
use bevy_app::{stage, App, EventReader, Events};
use bevy_ecs::{IntoQuerySystem, Local, Res, ResMut};

use bevy_prototype_simple_net::{
    IpAddress, NetPlugin, NetProtocol, OpenSocket, Port, SendSocket, SocketAddress, SocketError,
    SocketId,
};
...
fn setup(socket_id: Res<SocketId>, mut socket_open: ResMut<Events<OpenSocket>>) {
    ...
    socket_open.send(OpenSocket {
        ...
        port: if SOCKET_PROTOCOL == NetProtocol::Udp {
            Some(HOST_UDP_PORT)
        } else {
            None
        },
...
fn send_data_system(socket_id: Res<SocketId>, mut socket_send: ResMut<Events<SendSocket>>) {
...
fn handle_error_system(
    ...
    for socket_error_event in state.iter(&socket_error_events) {
        eprintln!(
            "Socket error (ID: {:?}): \"{:?}\"",
            socket_error_event.id, socket_error_event.err
        );
    }
}
